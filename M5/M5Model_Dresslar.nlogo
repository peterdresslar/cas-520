...
to setup
  ...
  set min-fert the-min-fertility / 100  ;; DRESSLAR
...
to setup-households
  ...
  create-households init-households [
    ...
    set energy init-energy
      ...
      set min-fertility min-fert ;; minimum acceptable fertility in a patch to farm it
...
to move
  ...
  set move-dist (random 4) + 1 ;; some stochasticity in move distances even in controlled mode
...
This model simulates some of the dynamics of house-hold level swidden agriculture (also called shifting cultivation or slash-and-burn agriculture). The user sets values related to farming decisions. 
...
### ESTABLISHING A FARM 
Households establish a farmstead and claim ownership of all patches within the [swidden-radius]. Other households cannot farm this land while it is is owned. The household then begins to farm patches in area owned. Farming is not permitted on the farmstead patch itself. Farmsteads are identified by their red color in controlled mode). 
...
### MOVING
If the energy of a household drops below a percentage of the initial energy ([init-move-threshold], the household will search for a new location to farm. It will look for a patch with a specified level of fertility and at some distance from the existing farmstead. In controlled mode, the household searches for land with at least 80% of the maximum fertility value within a radius that is [swidden_radius] X [a random number between 1 and 5]. 
...
Set options (see above). Most options are expressed in percent of initial household energy. The switch for [transfer-ownership] can be enabled or disabled.
...
<experiments>
  ...
  <experiment name="Long Evolution Run - Farm Cost Variation" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="randotime" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
